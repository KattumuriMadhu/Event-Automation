...
Date(s): ${event.dates && event.dates.length > 1 ? event.dates.map(d => new Date(d).toDateString()).join(', ') : (event.date ? new Date(event.date).toDateString() : 'TBA')}
...
export async function suggestPostingTime(event) {
  ...
  const prompt = `
    ...
    Audience: ${event.audience}
    Type: ${event.type}
    Date(s): ${event.dates && event.dates.length > 1 ? event.dates.join(', ') : (event.date || 'TBA')}
